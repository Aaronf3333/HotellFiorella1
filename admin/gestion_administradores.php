...
$params = [];
$filter_query_string = '';
// Se filtra por RolID = 1 (Administrador) y Estado = '1' (Activo)
$where_sql = "WHERE u.RolID = 1 AND u.Estado = '1'"; 
...
$total_sql = "SELECT COUNT(*) FROM Usuario u JOIN Empleados e ON u.EmpleadoID = e.EmpleadoID JOIN Persona p ON e.PersonaID = p.PersonaID $where_sql";
$stmt_total = $pdo->prepare($total_sql);
$stmt_total->execute($params);
$total_registros = $stmt_total->fetchColumn();
$total_paginas = ceil($total_registros / $registros_por_pagina);

$sql = "SELECT u.UsuarioID, u.NombreUsuario AS Email, p.Nombres, p.Ape_Paterno, p.Ape_Materno
        FROM Usuario u
...
<div style="display: flex; justify-content: space-between; align-items: center;">
    ...
    <a href="add_administrador.php" class="btn btn-success">Agregar Nuevo Administrador</a>
</div>


<div class="filter-bar" style="margin-bottom:20px; background-color:#fff; padding:15px; border-radius: 8px; margin-top: 15px;">
    <form action="gestion_administradores.php" method="GET" style="display:flex; gap: 15px; align-items:center;">
        ...
        <a href="gestion_administradores.php" class="btn btn-secondary">Limpiar</a>
    </form>
</div>
...
<table>
    ...
    <tbody>
        <?php if (count($admins) > 0): ?>
            ...
            <tr>
                <td><?php echo htmlspecialchars($admin['Nombres'] . ' ' . $admin['Ape_Paterno'] . ' ' . $admin['Ape_Materno']); ?></td>
                <td><?php echo htmlspecialchars($admin['Email']); ?></td>
                <td>
                    <a href="edit_usuario.php?id=<?php echo $admin['UsuarioID']; ?>" class="btn btn-primary">Editar</a>
                </td>
            </tr>
            <?php endforeach; ?>
        <?php else: ?>
            <tr><td colspan="3" style="text-align:center;">No se encontraron administradores.</td></tr>
        <?php endif; ?>
    </tbody>
</table>
